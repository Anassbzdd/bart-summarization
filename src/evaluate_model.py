...
from config import DATASET_NAME, DATASET_VERSION, MODEL_NAME, TEST_SIZE
...
def load_test_data(test_size):
    ...
    filtered = dataset["test"].filter(_filter_example)
    return filtered.select(range(min(test_size, len(filtered))))
...
def main():
    ...
    for example in test_data:
        predictions.append(summarize(example["article"], model, tokenizer, device, args))
        references.append(example["highlights"])

    metrics = rouge.compute(predictions=predictions, references=references, use_stemmer=True)
    output = {
